...
const EmployeesPage = () => {
  ...
  const getDatosTable = useCallback(async () => {
    try {
      ...
      alert('Ocurrio un error al recuperar los elementos del directorio intente de nuevo mas tarde')
      setProcesando(false)
    }
  }, [dispatch]);
  ...
  const eliminar = async (data:any) => {
    if(window.confirm('desea eliminar de forma permanente este registro?')){
      try {
        setProcesando(true)
        const response: any = await elimina(data?.id);
        setFilterText('')
  ...
  const editar = (data:any) => {
    setFilterText('')
    navigate(`/dierctorio-edit`, { state: { data } });
  }

  return (
    <div>
      <Header inSession={inSession ? true : false} />
      {!procesando && <Container fluid>
        <Search
        ...
        <div style={{ width:'100%' }}>
        <TableComponent key={filteredItems?.length} data={filteredItems} onAction={(action,data)=>action === 'editar' ? editar(data) : eliminar(data)}/>
        </div>
      </Container>}
      {procesando && <Container fluid><div style={{ width:'100%', textAlign:'center' }}><h1>Procesando...</h1></div></Container>}
    </div>
  );
};
