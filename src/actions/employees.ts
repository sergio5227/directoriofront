...
export const editEmployee = async (data:any) => {
  try {
    const response: GeneralHttpResponse = await axios.post(
      `${"http://127.0.0.1/directorioAPI/public/api/actualizaElemento/"}${data?.id}`, data, {
        headers: {
...
export const saveEmployee = async (data:any) => {
  try {
    const response: GeneralHttpResponse = await axios.post(
      `${"http://127.0.0.1/directorioAPI/public/api/altaElemento"}`, data, {
        headers: {
          "Content-Type": "multipart/form-data",
        },
      }
...
export const elimina = async (id:any) => {
  try {
    const response: GeneralHttpResponse = await axios.post(
      `${"http://127.0.0.1/directorioAPI/public/api/eliminaElemento/"}${id}`
    );
    return response?.data || [];
  } catch (error) {
    const promise = new Promise((resolve, reject) => reject(error));
